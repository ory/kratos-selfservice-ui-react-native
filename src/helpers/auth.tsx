...
import { LoginViaApiResponse, RegistrationViaApiResponse, Session as KratosSession } from '@oryd/kratos-client';
...
export const getAuthenticatedSession = (): Promise<Session | null> =>
  SecureStore
    .getItemAsync(userSessionName)
    .then((sessionRaw: string | null) => {
      if (!sessionRaw) {
        return null;
      }
...
// Sets the session.
export const setAuthenticatedSession = (session: RegistrationViaApiResponse | LoginViaApiResponse) => {
  if (!session.sessionToken) {
    return Promise.reject(new Error('Expected the session to have a session token but received none.'));
  }
